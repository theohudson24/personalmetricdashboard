...
export function DataPrivacyControls({ request }: { request: { id: string; status: string } | null }) {
  ...
  return <Card><CardHeader title="Your data" description="Download a complete copy or request administrator-reviewed account deletion."/>
    <a href="/api/account/export" className="inline-flex min-h-11 items-center rounded-md border border-core bg-core px-4 text-sm font-medium text-white"><Download size={16}/><span className="ml-2">Download ZIP export</span></a>
