...
export function AccountSettings({ account }: { account: { displayName: string; email: string; emailVerified: boolean; phone: string } }) {
  ...
  return <Card>
    ...
    <div className="mt-5 border-t border-line pt-5"><Button type="button" variant="secondary" className="transition-[color,background-color,border-color,box-shadow] hover:border-ember/70 hover:bg-ember/10 hover:text-ember hover:shadow-[0_0_24px_rgba(201,101,95,0.24)] focus-visible:border-ember focus-visible:outline-ember" onClick={() => void logOut()} disabled={loggingOut}><LogOut size={16}/><span className="ml-2">{loggingOut ? "Logging out…" : "Log out of this account"}</span></Button>{logoutError ? <p className="mt-2 text-sm text-ember" role="alert">{logoutError}</p> : null}</div>
  </Card>;
}
