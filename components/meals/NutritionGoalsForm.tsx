...
export function NutritionGoalsForm({ settings, profile, recommendation, showProfileMetrics = false }: {
  ...
  return (
    <Card>
      ...
      {pendingHref ? <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" role="dialog" aria-modal="true" aria-labelledby="settings-leave-title"><div className="w-full max-w-lg rounded-lg border border-line bg-panel p-5 shadow-2xl"><h3 id="settings-leave-title" className="text-lg font-semibold">Unsaved settings</h3><p className="mt-2 text-sm text-muted">Review the profile information, then stay here or retry saving before continuing.</p><dl className="mt-4 grid grid-cols-2 gap-2 rounded-md border border-line bg-black/15 p-3 text-sm"><dt className="text-muted">Height</dt><dd>{profileValues.heightFeet || "–"} ft {profileValues.heightInchesRemainder || "–"} in</dd><dt className="text-muted">Weight</dt><dd>{profileValues.weightLb || "–"} lb</dd><dt className="text-muted">Age</dt><dd>{profileValues.age || "–"}</dd><dt className="text-muted">Gender</dt><dd>{profileValues.gender || "–"}</dd><dt className="text-muted">Targets</dt><dd>{goalMode === "recommended" ? "Recommended estimates" : "Customized"}</dd></dl>{saveState === "error" || saveState === "conflict" ? <p className="mt-3 text-sm text-ember">{saveMessage}</p> : null}<div className="mt-5 flex justify-end gap-2"><button type="button" className="min-h-11 rounded-md border border-line px-4 text-sm" onClick={() => setPendingHref(null)}>Stay on page</button><button type="button" disabled={saveState === "saving"} className="min-h-11 rounded-md border border-core bg-core px-4 text-sm font-medium text-[#07100d] disabled:opacity-50" onClick={async () => { const destination = pendingHref; if (await persist(latestRef.current)) window.location.assign(destination); }}>{saveState === "saving" ? "Saving…" : "Save and continue"}</button></div></div></div> : null}
    </Card>
  );
}
