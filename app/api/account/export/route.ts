...
export async function GET() {
  ...
  const [settings, meals, savedFoods, workouts, habits, goals, routines, checklist, reviews, bugReports, deletionRequests] = await Promise.all([
    ...
    prisma.selfImprovementGoal.findMany({ where: { profileId: profile.id }, include: { habitLinks: true, routineLinks: true, progressEntries: { orderBy: { recordedAt: "asc" } } }, orderBy: { createdAt: "desc" } }),
