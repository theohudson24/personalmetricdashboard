...
import { defaultChecklist, goalPaceStatus, goalProgressPercent, improvementCategories, todayUtc, uniqueSuccessfulHabitDays, weekStartUtc } from "@/lib/selfImprovement";
...
export default async function SelfImprovementPage() {
  ...
  return <div><PageHeader eyebrow="Sustainable personal growth" title="Self-Improvement" description="Build confidence, health, presentation, and self-respect through realistic routines and measurable actions."/><div className="grid gap-5"><SafetyNotice/><SelfImprovementOverview completion={completion} weeklyConsistency={weeklyConsistency} activeGoals={activeGoals} completedGoals={completedGoals} streak={completion>0?1:0} focus={focus} strongest={strongest}/><div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]"><DailyImprovementChecklist items={todayItems} categories={categories}/><Card><CardHeader title="Optional suggestions" description="Generated from your logged activity using simple heuristics—not medical findings or guaranteed outcomes."/><div className="space-y-3">{recommendations.map((recommendation,index)=><div key={recommendation} className="flex gap-3 rounded-md border border-line bg-black/15 p-3"><Lightbulb size={17} className="mt-0.5 shrink-0 text-growth"/><div><p className="text-xs font-semibold uppercase tracking-wider text-muted">Optional suggestion {index+1}</p><p className="mt-1 text-sm leading-6">{recommendation}</p></div></div>)}</div><div className="mt-4 flex gap-2 rounded-md border border-line p-3 text-xs leading-5 text-muted"><LockKeyhole size={17} className="shrink-0 text-core"/>Your self-improvement records are tied to your account. Photos are not enabled until private object storage and deletion controls are implemented.</div></Card></div><CategoryGrid categories={improvementCategories} activity={categoryActivity}/><div className="grid gap-5 xl:grid-cols-2"><ImprovementGoals goals={goals} categories={categories} habits={habits} routines={routines.map(({id,name})=>({id,name}))}/><RoutineBuilder routines={routines} categories={categories}/></div>{metrics.length?<Card><CardHeader title="Personal metrics" description="Category-specific markers only—never an attractiveness score, diagnosis, or comparison with other people."/><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{metrics.map(metric=><div key={metric.id} className="rounded-md border border-line bg-black/15 p-3"><TrendingUp size={17} className="text-core"/><p className="mt-2 text-sm text-muted">{metric.category}</p><p className="font-semibold">{metric.name}: {metric.value}{metric.unit}</p></div>)}</div></Card>:null}<WeeklyImprovementReview review={review} previousAdjustment={previousReview?.adjustment ?? null} weekStart={weekStartUtc()} categories={categories}/></div></div>;
}
